...
window.addEventListener('DOMContentLoaded', async function() {
    ...
    const animeId = urlParams.get('mal_id') || urlParams.get('season_id');
...
function renderAnimeDetail(anime) {
    ...
    if (statusElement) {
        statusElement.textContent = anime.status || '未知';
    }
    ...
    // 设置详细信息
    document.getElementById('anime-aired').textContent = anime.aired?.string || '未知';
    ...
    document.getElementById('anime-producers').textContent = anime.producers?.map(p => p.name).join(', ') || '未知';
    document.getElementById('anime-source').textContent = anime.source || '未知';
    document.getElementById('anime-themes').textContent = anime.themes?.map(t => t.name).join(', ') || '未知';
    document.getElementById('anime-demographics').textContent = anime.demographics?.map(d => d.name).join(', ') || '未知';
    document.getElementById('anime-duration').textContent = anime.duration || '未知';
